...
namespace SecureShare.Website.Controllers
{
    public class FileReader
    {
        ...
        public async Task<ResultAuth> Authenticate(string pathToUser, string userName)
        {
            ...
            await faceServiceClient.DeletePersonGroupAsync(_options.PersonGroupId);
            await faceServiceClient.CreatePersonGroupAsync(_options.PersonGroupId, "SecureShareGroup");
            using (Stream s = File.OpenRead(pathToUser))
            {
                ...
                if (!found)
                {
                    ...
                    pId = person.PersonId;
                    await faceServiceClient.AddPersonFaceAsync(_options.PersonGroupId, pId, s);
                    await faceServiceClient.TrainPersonGroupAsync(_options.PersonGroupId);

                    return ra = new ResultAuth(1);
                }
                ...
                return ra = new ResultAuth(res.Confidence);
            }
        }
    }
}
